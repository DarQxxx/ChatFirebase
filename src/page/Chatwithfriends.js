...
export default function Chatwithfriends() {
  ...
  function sendMessageWithEnter(e){
    if (e.key === "Enter" && (message !== "")){
      ...
      if(myuid !== params.hisuid){ msg2.add({
        text: message,
        createdAt: time(),
        uid: myuid,
 ...
 <div>
    ...
    <div className="container">
          ...
          {firebaseFriendList.map((friends, index) =>(
            <div>
              {friends.uid !== myuid && <Link key={index} onClick={() => {setIsLoading(true)}} to={`/chat/${friends.uid}`}><div className="text-center pt-3"><img className="friends-img" src={friends.url} alt=""/></div></Link>}
            </div>
           ...
           {/*   {chatMessages.map((message, index)=> (<div className="messages-every messages-my" key={index}><div className="message-direct message-direct-my">{message}</div></div>))} */}
            {fireBaseMessages.map((msg, index) => (
              <div key={index}>
                {msg.uid === myuid ? (<div className="messages-every messages-my"  key={msg.id}>
